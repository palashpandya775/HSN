...
/* stored in the memory. two parameter optimization.                     */
/*************************************************************************/
...
void generate_rho0(gsl_matrix_complex * rho0, int dim, int dimn, int np, char * nm ){ //Maximally entangled n qudit state
	int i, offset=0;
	gsl_matrix_complex  *cb2[dim];
	
	for(i=1; i<=np; i++){
		offset += (int) pow(dim, np-i);
	}
	cb2[0] = gsl_matrix_complex_calloc(dimn, 1);
	gsl_matrix_complex_set(cb2[0], 0,0, GSL_COMPLEX_ONE);
	for(i=1; i<dim; i++){
		cb2[i] = gsl_matrix_complex_calloc(dimn, 1);
		gsl_matrix_complex_set(cb2[i], i*offset,0, GSL_COMPLEX_ONE);
		gsl_matrix_complex_add(cb2[0], cb2[i]);
	}

	gsl_blas_zgemm(CblasNoTrans, CblasConjTrans, GSL_COMPLEX_ONE, cb2[0], cb2[0], GSL_COMPLEX_ONE, rho0);

	gsl_matrix_complex_scale(rho0, gsl_complex_rect(1./(double)dim, 0.));

	for(i=0; i<dim; i++){
		gsl_matrix_complex_free(cb2[i]);
	}
	return;
}
